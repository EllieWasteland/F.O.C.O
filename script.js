document.addEventListener('DOMContentLoaded', () => {
    ...
    function applyAppearance() { 
        document.body.style.backgroundImage = `url('${state.wallpaper || "https://raw.githubusercontent.com/lucasromerodb/liquid-glass-effect-macos/refs/heads/main/assets/flowers.jpg"}')`; 
        document.documentElement.style.setProperty('--accent-primary', state.zenSettings.color); 
        inputs.colorPicker.value = state.zenSettings.color;
    }
        ...
        return new Promise((resolve, reject) => {
            ...
            img.onload = () => {
                ...
                try {
                    ...
                    reject(new Error("Could not get image data for color analysis."));
                    return;
                }
                ...
                for (let i = 0; i < imageData.length; i += 4 * 5) { // Sample every 5th pixel
                    ...
                    // Filter out very dark/light colors and grays
                    const luminance = 0.299 * r + 0.587 * g + 0.114 * b;
                    const saturation = Math.max(r, g, b) - Math.min(r, g, b);
                    if (luminance < 40 || luminance > 215 || saturation < 20) continue;
                ...
                if (dominantColor) {
                    ...
                    resolve('#00F0FF'); // Fallback color
                }
            };
            img.onerror = () => {
                reject(new Error("Could not load image for color analysis."));
            };
        });
    }

    async function handleWallpaperUrl() { 
        const url = inputs.wallpaperUrl.value.trim(); 
        if (url) { 
            state.wallpaper = url; 
            applyAppearance();
            try {
                const color = await getDominantColor(url);
                state.zenSettings.color = color;
                applyAppearance();
            } catch (e) {
                console.error(e);
            }
    ...
    function handleWallpaperFile(event) { 
        ...
        reader.onload = async e => { 
            ...
            applyAppearance();
            try {
                const color = await getDominantColor(e.target.result);
                state.zenSettings.color = color;
                applyAppearance();
            } catch (e) {
                console.error(e);
    ...
    async function resetWallpaper() { 
        ...
        state.zenSettings.color = '#00F0FF';
        await dataService.saveData(state); 
        applyAppearance(); 
        inputs.wallpaperUrl.value = ''; 
        inputs.wallpaperFile.value = ''; 
    }
    ...
    function handleFullscreenChange() {
        if (!document.fullscreenElement && views.zen.classList.contains('active')) {
            stopZenMode();
        }
    }
    ...
    function hideHiddenView(viewId) {
        if (views[viewId]) {
            views[viewId].classList.remove('active');
        }
    }
    ...
    function openSubtaskModal() { 
        modal.subtask.element.style.zIndex = '200';
        modal.subtask.element.classList.add('active'); 
        modal.subtask.input.value = ''; 
        modal.subtask.input.focus(); 
    }

    function closeSubtaskModal() { 
        modal.subtask.element.classList.remove('active'); 
    }
    ...
    function openEditSubjectForm() {
        ...
        hideHiddenView('grades');
        views.schedule.scrollIntoView({ behavior: 'smooth' });
    ...
    function renderCalendar() { 
        const calDate = new Date(state.calendarDate); 
        const y = calDate.getFullYear(), m = calDate.getMonth(); 
        containers.calendarTitle.textContent = calDate.toLocaleDateString('es-ES', { month: 'long', year: 'numeric' }).toUpperCase(); 
        const firstDay = new Date(y, m, 1).getDay(); 
        const daysInMonth = new Date(y, m + 1, 0).getDate(); 
        containers.calendarGrid.innerHTML = ['D', 'L', 'M', 'X', 'J', 'V', 'S'].map(d => `<div class="font-bold text-secondary">${d}</div>`).join(''); 
        for (let i = 0; i < firstDay; i++) containers.calendarGrid.innerHTML += `<div></div>`; 
        const tasksByDate = {}; 
        state.tasks.forEach(task => { if(task.dueDate){ const date = task.dueDate.split('T')[0]; if(!tasksByDate[date]) tasksByDate[date] = []; tasksByDate[date].push(task); }}); 
        ...
        for (let day = 1; day <= daysInMonth; day++) { 
            ...
            if (tasksByDate[fullDate]) el.classList.add('has-task'); 
            el.addEventListener('click', () => { 
                document.querySelectorAll('#calendar-grid > div').forEach(d => d.classList.remove('selected')); 
                el.classList.add('selected'); 
                renderTasksForDay(fullDate, tasksByDate[fullDate] || []); 
            }); 
            containers.calendarGrid.appendChild(el); 
        } 
    }
    ...
    // --- EVENT HANDLERS ---
    formElements.setupForm.addEventListener('submit', async (e) => { e.preventDefault(); const name = inputs.username.value.trim(); if(name){ state.userName = name; await dataService.saveData(state); views.setup.style.display = 'none'; mainAppUI.style.display = 'block'; initApp(); } });
    buttons.fabAddTask.addEventListener('click', openNewTaskForm);
    ...
    buttons.cancelForm.addEventListener('click', () => { hideHiddenView('form'); });
    ...
    buttons.backToSchedule.addEventListener('click', () => hideHiddenView('grades'));
